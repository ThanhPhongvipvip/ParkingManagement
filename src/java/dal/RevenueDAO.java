...
public class RevenueDAO extends DBContext {
    ...
    public double getRevenueByType(String type, String fromDate, String toDate) {
        ...
        if ("Ticket".equals(type)) {
            ...
            if (fromDate != null && !fromDate.isEmpty()) sql += " AND CAST(CheckOutTime AS DATE) >= ? ";
            if (toDate != null && !toDate.isEmpty()) sql += " AND CAST(CheckOutTime AS DATE) <= ? ";
        } else {
            sql = "SELECT SUM(Price) AS Total FROM MonthlyPasses WHERE 1=1 ";
            if (fromDate != null && !fromDate.isEmpty()) sql += " AND CAST(CreatedAt AS DATE) >= ? ";
            if (toDate != null && !toDate.isEmpty()) sql += " AND CAST(CreatedAt AS DATE) <= ? ";
        }

        try {
            PreparedStatement st = connection.prepareStatement(sql);
            int paramIndex = 1;
            if (fromDate != null && !fromDate.isEmpty()) st.setString(paramIndex++, fromDate);
            if (toDate != null && !toDate.isEmpty()) st.setString(paramIndex, toDate);
